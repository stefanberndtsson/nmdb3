...
#      params.delete(key)
  ...
  def image_view
    ...
    if @type == "movie"
      ...
      @image_url = @movie.image_url(view_context.current_user, true) # WikipediaFetcher.image(@movie)
      @title = @movie.display
    elsif @type == "person"
      @person = Person.find(@object_id)
      @image_url = @person.image_url(view_context.current_user, true) # WikipediaFetcher.image(@person)
